...
public static void CupImgDrawColor(ref byte[] bSrcImg, int nImageW3,int nMinX, int nMaxX, int nMinY, int nMaxY,byte ucR, byte ucG, byte ucB)
{
    ...
    {
        nX3=nMinX*3;
        for(int i=nMinX;i<=nMaxX;i++,nX3+=3)
        {
	...
	try
	{
		...
		for (int k = 0; k < nCupNum; k++)
		{
			if(nChannelHs[1]>0&&nChannelHs[2]>0)
			{
                ...
                CupImgColorStatistic(ref bSrcImg, nImageW, nCutLeft[k], nCutRight[k], nMinY[k], nMaxY[k],ColorType, colorRGB, unColorIntervals, ref nTempColorCount);	
                ...
                CupImgColorStatistic(ref bSrcImgRight, nImageW,nCutLeft2[k], nCutRight2[k], nMinY2[k], nMaxY2[k], ColorType, colorRGB, unColorIntervals, ref nTempColorCount);	
			...
			else
			{
                ...
                GetCupImgOutRect(bSrcImgMid, nImageW, nChannelHs[0], nLefts0[k], nLefts0[k + 1], ref nMinX[k], ref nMaxX[k], ref nMinY[k], ref nMaxY[k]);
				nCx=nMinX[k]+(nMaxX[k]-nMinX[k]>>1);
				if (k==0)
				{
					nPerimeter=(int)(3.1415926535897932384626433832795f*(nMaxX[k]-nMinX[k]));
				...
				nCutLeft[k]=nCx-nMidLen;
				nCutRight[k]=(nCx+nMidLen)<(nCutLeft[k]+nPerimeter-(k*nMidLen<<1))?(nCx+nMidLen):(nCutLeft[k]+nPerimeter-(k*nMidLen<<1));
                CupImgColorStatistic(ref bSrcImgMid, nImageW, nCutLeft[k], nCutRight[k], nMinY[k], nMaxY[k], ColorType, colorRGB, unColorIntervals, ref nTempColorCount);	
