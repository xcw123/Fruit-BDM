...
namespace FruitSortingVtest1._0
{
    public partial class WaveCaptureForm : Form
    {
        ...
        Point[] m_wave1Point = new Point[768];
        Point[] m_wave2Point = new Point[768];
        ...
        private void DrawPictureBox(Graphics graphics)
        {
            try
            {
                ...
                graphics.DrawLine(pen, this.WeightWavepictureBox.Width / 3 + m_waveInterval[0], 0, this.WeightWavepictureBox.Width / 3 + m_waveInterval[0], this.WeightWavepictureBox.Height);
                graphics.DrawLine(pen, this.WeightWavepictureBox.Width / 3 + m_waveInterval[1], 0, this.WeightWavepictureBox.Width / 3 + m_waveInterval[1], this.WeightWavepictureBox.Height);

                m_Intervalrect[0] = new Rectangle(this.WeightWavepictureBox.Width / 3 + m_waveInterval[0] - rectangleWidth / 2, this.WeightWavepictureBox.Height / 2 - rectangleWidth / 2, rectangleWidth, rectangleWidth);
                SolidBrush brush = new SolidBrush(Color.Yellow);//定义画刷
                graphics.FillRectangle(brush, m_Intervalrect[0]);//填充矩形

                m_Intervalrect[1] = new Rectangle(this.WeightWavepictureBox.Width / 3 + m_waveInterval[1] - rectangleWidth / 2, this.WeightWavepictureBox.Height / 2 - rectangleWidth / 2, rectangleWidth, rectangleWidth);
        ...
        private void WeightWavepictureBox_MouseMove(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                if ((e.X >= m_Intervalrect[0].X) && (e.X <= m_Intervalrect[0].X + m_Intervalrect[0].Width) && (e.Y >= m_Intervalrect[0].Y) && (e.Y <= m_Intervalrect[0].Y + m_Intervalrect[0].Height))
                {
                    ...
                    {
                        m_tempInterval = m_waveInterval[0];
                ...
                else if ((e.X >= m_Intervalrect[1].X) && (e.X <= m_Intervalrect[1].X + m_Intervalrect[1].Width) && (e.Y >= m_Intervalrect[1].Y) && (e.Y <= m_Intervalrect[1].Y + m_Intervalrect[1].Height))
                {
                    ...
                    if (m_MouseDown && !m_IntervalCatched)
                    {
                        m_tempInterval = m_waveInterval[1];
                                ...
                                for (int i = 0; i < 256; i++)
                                {
                                    if (GlobalDataInterface.globalIn_wavelist.Count < 4)
                                    {
                                        if (GlobalDataInterface.globalIn_wavelist.Count == j)
                                            break;
                                        m_wave1Point[(2 - k) * 256 + i].X = 256 * (2 - k) + i;
                                        if (GlobalDataInterface.globalIn_wavelist[j].waveform0[i] < 0)
                                            GlobalDataInterface.globalIn_wavelist[j].waveform0[i] = 0;
                                        if (GlobalDataInterface.globalIn_wavelist[j].waveform0[i] > 4095)
                                            GlobalDataInterface.globalIn_wavelist[j].waveform0[i] = 4095;
                                        m_wave1Point[(2 - k) * 256 + i].Y = (int)((4096 - GlobalDataInterface.globalIn_wavelist[j].waveform0[i]) * m_fTransRatio);

                                        m_wave2Point[(2 - k) * 256 + i].X = 256 * (2 - k) + i;
                                        if (GlobalDataInterface.globalIn_wavelist[j].waveform1[i] < 0)
                                            GlobalDataInterface.globalIn_wavelist[j].waveform1[i] = 0;
                                        if (GlobalDataInterface.globalIn_wavelist[j].waveform1[i] > 4095)
                                            GlobalDataInterface.globalIn_wavelist[j].waveform1[i] = 4095;
                                        m_wave2Point[(2 - k) * 256 + i].Y = (int)((4096 - GlobalDataInterface.globalIn_wavelist[j].waveform1[i]) * m_fTransRatio + this.WeightWavepictureBox.Height / 2);
                                    }
                                    else
                                    {
                                        m_wave1Point[(2 - k) * 256 + i].X = 256 * (2 - k) + i;
                                        ...
                                        if (GlobalDataInterface.globalIn_wavelist[this.WaveCapturehScrollBar.Value - k + 2].waveform0[i] > 4095)
                                            GlobalDataInterface.globalIn_wavelist[this.WaveCapturehScrollBar.Value - k + 2].waveform0[i] = 4095;
                                        m_wave1Point[(2 - k) * 256 + i].Y = (int)((4096 - GlobalDataInterface.globalIn_wavelist[this.WaveCapturehScrollBar.Value - k + 2].waveform0[i]) * m_fTransRatio);
                                        ...
                                        if (GlobalDataInterface.globalIn_wavelist[this.WaveCapturehScrollBar.Value - k + 2].waveform1[i] > 4095)
                                            GlobalDataInterface.globalIn_wavelist[this.WaveCapturehScrollBar.Value - k + 2].waveform1[i] = 4095;
                                        m_wave2Point[(2 - k) * 256 + i].Y = (int)((4096 - GlobalDataInterface.globalIn_wavelist[this.WaveCapturehScrollBar.Value - k + 2].waveform1[i]) * m_fTransRatio + this.WeightWavepictureBox.Height / 2);
                                    }
            ...
            try
            {
                ...
                if (scrollBar.Value > 0)
                {
                    for (int j = 0; j < 3; j++)
                    {
                        for (int i = 0; i < 256; i++)
                        {
                            m_wave1Point[(2 - j) * 256 + i].X = 256 * (2 - j) + i;
                            ...
                            if (GlobalDataInterface.globalIn_wavelist[scrollBar.Value - j + 2].waveform0[i] > 4095)
                                GlobalDataInterface.globalIn_wavelist[scrollBar.Value - j + 2].waveform0[i] = 4095;
                            m_wave1Point[(2 - j) * 256 + i].Y = (int)((4096 - GlobalDataInterface.globalIn_wavelist[scrollBar.Value - j + 2].waveform0[i]) * m_fTransRatio);

                            m_wave2Point[(2 - j) * 256 + i].X = 256 * (2 - j) + i;
                            ...
                            if (GlobalDataInterface.globalIn_wavelist[scrollBar.Value - j + 2].waveform1[i] > 4095)
                                GlobalDataInterface.globalIn_wavelist[scrollBar.Value - j + 2].waveform1[i] = 4095;
                            m_wave2Point[(2 - j) * 256 + i].Y = (int)((4096 - GlobalDataInterface.globalIn_wavelist[scrollBar.Value - j + 2].waveform1[i]) * m_fTransRatio + this.WeightWavepictureBox.Height / 2);
