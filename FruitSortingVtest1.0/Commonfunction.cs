...
namespace Common
{
    public static class Commonfunction
    {
        ...
        /// </summary>
        /// <param name="x"></param>子系统
        /// <param name="y"></param>IPM、通道,版本3.2
        /// <returns></returns>
        public static int EncodeIPMChannel(int x, int y)
        {
            int result = ((x + 1) << 8 | ((y >> 1) + 1) << 4 | ((y & 1) + 1));
            return result;
        }
                ...
                for (int i = 0; i < ConstPreDefine.MAX_SUBSYS_NUM; i++)
                {
                    for (int j = 0; j < ConstPreDefine.MAX_CHANNEL_NUM; j++)
                    {
                        ...
                        if (bChannelInfo[i] > j)  //Modify by ChengSk - 20190521
                        {
                            ...
                            {
                                ID = EncodeChannel(i, j, j);
                            }
                            else if (GlobalDataInterface.nVer == 1)
                            {
                                ID = EncodeChannel(i, j / 2, j % 2);
                            }
                            arrayID.Add(ID);
                        }
                    }
        ...
        public static void GetAllProjectSettingFileName(ref List<string> configFileNameList)
        {
            try
            {
                ...
                DirectoryInfo dir = new DirectoryInfo(System.Environment.CurrentDirectory + "\\config\\" + GlobalDataInterface.VERSION_SHOW);
        ...
        ****************************/
        ...
        public static void GetCupImgOutRect_Gray(byte[] bBinImg, int nImageW, int nImageH, int nLeft, int nRight, ref int nMinX, ref int nMaxX, ref int nMinY, ref int nMaxY)
        {
            bool bExist;
            int nOffsetTemp, nOffset;

            //求nMinY
            nOffset = 0;
            nMinY = 0;
            bExist = false;
            for (; nMinY < nImageH; nMinY++, nOffset += nImageW)
            {
                for (int x = nLeft; x < nRight; x++)
                {
                    if (bBinImg[nOffset + x] > 0)
                    {
                        bExist = true;
                        break;
                    }
            ...
            //求nMaxY
            nMaxY = nImageH - 1;
            nOffset = (nImageH - 1) * nImageW;
            bExist = false;
            for (; nMaxY >= nMinY; nMaxY--, nOffset -= nImageW)
            {
                for (int x = nLeft; x < nRight; x++)
                {
                    if (bBinImg[nOffset + x] > 0)
            ...
            nMinX = nLeft;
            bExist = false;
            nOffset = nMinY * nImageW;
            for (; nMinX < nRight; nMinX++)
            {
                nOffsetTemp = nOffset;
                for (int y = nMinY; y <= nMaxY; y++, nOffsetTemp += nImageW)
                {
                    if (bBinImg[nOffsetTemp + nMinX] > 0)
                    {
                        bExist = true;
                        break;
                    }
            ...
            bExist = false;
            nOffset = nMinY * nImageW;
            for (; nMaxX >= nMinX; nMaxX--)
            {
                nOffsetTemp = nOffset;
                for (int y = nMinY; y <= nMaxY; y++, nOffsetTemp += nImageW)
                {
                    if (bBinImg[nOffsetTemp + nMaxX] > 0)
                    {
                        bExist = true;
                        break;
                    }
        ...
        public static void CupImgColorStatistic(ref byte[] bSrcImg, int nImageW3, int nMinX, int nMaxX, int nMinY, int nMaxY, int ColorType, ColorRGB[] colorRGB, uint[] unColorIntervals, ref int[] ColorTemp)
        {
            int x, y, nX3, nOffset3, nH1, nH, nMinVal, nMaxVal, nB, nG, nR, nU, nV;
            ...
            nOffset3 = nMinY * nImageW3;
            for (y = nMinY; y <= nMaxY; y++, nOffset3 += nImageW3)
            {
                nX3 = nMinX * 3;
                for (x = nMinX; x <= nMaxX; x++, nX3 += 3)
                {
                    nB = bSrcImg[nOffset3 + nX3];
                    nG = bSrcImg[nOffset3 + nX3 + 1];
                    nR = bSrcImg[nOffset3 + nX3 + 2];
                    if (nB > 0 || nG > 0 || nR > 0)
                    {
                        switch (ColorType)
                        {
                            case RATIO_UV:
                                ...
                                else if ((U0 <= nU && nU <= U1) && (V0 <= nV && nV <= V1))
                                {
                                    ...
                                    continue;
                                bSrcImg[nOffset3 + nX3] = ucB;
                                bSrcImg[nOffset3 + nX3 + 1] = ucG;
                                bSrcImg[nOffset3 + nX3 + 2] = ucR;
                                break;
                            case AVG_UV:
                                nU = (int)((COEFF_U0 * nR + COEFF_U1 * nG + COEFF_U2 * nB + SHERU) + 128);
                                nV = (int)((COEFF_V0 * nR + COEFF_V1 * nG + COEFF_V2 * nB + SHERU) + 128);
        ...
        public static void CupImgDrawColor(ref byte[] bSrcImg, int nImageW3, int nMinX, int nMaxX, int nMinY, int nMaxY, byte ucR, byte ucG, byte ucB)
        {
            int x, y, nX3, nOffset3;
            ...
            for (y = nMinY; y <= nMaxY; y++, nOffset3 += nImageW3)
            {
                ...
                for (x = nMinX; x <= nMaxX; x++, nX3 += 3)
                {
                    if (bSrcImg[nOffset3 + nX3] > 0 || bSrcImg[nOffset3 + nX3 + 1] > 0 || bSrcImg[nOffset3 + nX3 + 2] > 0)
                    {
                        bSrcImg[nOffset3 + nX3] = ucB;
                        bSrcImg[nOffset3 + nX3 + 1] = ucG;
                        bSrcImg[nOffset3 + nX3 + 2] = ucR;
        ...
        //根据收到的图获取lefts
        public static void GetLefts(byte[] bSrcImg, int nImageW, int[] nChannelHs, ref int[] nLefts1, ref int[] nLefts0, ref int[] nLefts2)
        {
            int x, y, nOffset, i;
            bool bstart;
            int nW3 = nImageW * 3;
            int[] nSumForeBin = new int[nImageW];
            if (nChannelHs[1] > 0)
            {
                nOffset = 0;
                Array.Clear(nSumForeBin, 0, nSumForeBin.Length);
                for (y = 0; y < nChannelHs[1]; y++, nOffset += nW3)
                {
                    for (x = 0; x < nImageW; x++)
                    {
                        if (bSrcImg[nOffset + x * 3] > 0)
                        {
                            nSumForeBin[x]++;
                        }
                    }
            ...
            if (nChannelHs[0] > 0)
            {
                nOffset = nChannelHs[1] * nW3;
                Array.Clear(nSumForeBin, 0, nSumForeBin.Length);
                for (y = 0; y < nChannelHs[0]; y++, nOffset += nW3)
                {
                    for (x = 0; x < nImageW; x++)
                    {
                        if (bSrcImg[nOffset + x * 3] > 0)
            ...
            if (nChannelHs[2] > 0)
            {
                nOffset = (nChannelHs[0] + nChannelHs[1]) * nW3;
                Array.Clear(nSumForeBin, 0, nSumForeBin.Length);
                for (y = 0; y < nChannelHs[2]; y++, nOffset += nW3)
                {
                    for (x = 0; x < nImageW; x++)
                    {
                        if (bSrcImg[nOffset + x * 3] > 0)
        ...
        public static void GetLefts_foreBin(byte[] bBinImg, int nImageW, int[] nChannelHs, ref int[] nLefts1, ref int[] nLefts0, ref int[] nLefts2)
        {
            int x, y, nOffset, i;
            bool bstart;
            int[] nSumForeBin = new int[nImageW];
            if (nChannelHs[1] > 0)
            {
                nOffset = 0;
                Array.Clear(nSumForeBin, 0, nSumForeBin.Length);
                for (y = 0; y < nChannelHs[1]; y++, nOffset += nImageW)
                {
                    for (x = 0; x < nImageW; x++)
                    {
                        if (bBinImg[nOffset + x] > 0)
            ...
            if (nChannelHs[0] > 0)
            {
                nOffset = nChannelHs[1] * nImageW;
                Array.Clear(nSumForeBin, 0, nSumForeBin.Length);
                for (y = 0; y < nChannelHs[0]; y++, nOffset += nImageW)
                {
                    for (x = 0; x < nImageW; x++)
                    {
                        if (bBinImg[nOffset + x] > 0)
            ...
            if (nChannelHs[2] > 0)
            {
                nOffset = (nChannelHs[0] + nChannelHs[1]) * nImageW;
                Array.Clear(nSumForeBin, 0, nSumForeBin.Length);
                for (y = 0; y < nChannelHs[2]; y++, nOffset += nImageW)
                {
                    for (x = 0; x < nImageW; x++)
                    {
                        if (bBinImg[nOffset + x] > 0)
        ...
        public static void ColorStatistic24(ref byte[] bSrcImg, int nImageW, int[] nChannelHs, int nCupNum, int nMidLen, float fCutY, int ColorType,
         ColorRGB[] colorRGB, uint[] unColorIntervals, ref int[] ColorCount)
        {
            int[] nLefts1 = new int[ConstPreDefine.MAX_SPLICE_CUP_NUM];
            int[] nLefts0 = new int[ConstPreDefine.MAX_SPLICE_CUP_NUM];
            int[] nLefts2 = new int[ConstPreDefine.MAX_SPLICE_CUP_NUM];
            GetLefts(bSrcImg, nImageW, nChannelHs, ref nLefts1, ref nLefts0, ref nLefts2);
            ...
            try
            {
                ...
                for (int k = 0; k < nCupNum; k++)
                {
                    if (nChannelHs[1] > 0 && nChannelHs[2] > 0)
                    {
                        ...
                        CupImgColorStatistic(ref bSrcImg, nW3, nCutLeft[k], nCutRight[k], nMinY[k], nMaxY[k], ColorType, colorRGB, unColorIntervals, ref nTempColorCount);
                        ...
                        CupImgColorStatistic(ref bSrcImgRight, nW3, nCutLeft2[k], nCutRight2[k], nMinY2[k], nMaxY2[k], ColorType, colorRGB, unColorIntervals, ref nTempColorCount);
                    ...
                    else
                    {
                        ...
                        nCutLeft[k] = nCx - nMidLen;
                        nCutRight[k] = (nCx + nMidLen) < (nCutLeft[k] + nPerimeter - (k * nMidLen << 1)) ? (nCx + nMidLen) : (nCutLeft[k] + nPerimeter - (k * nMidLen << 1));
                        CupImgColorStatistic(ref bSrcImgMid, nW3, nCutLeft[k], nCutRight[k], nMinY[k], nMaxY[k], ColorType, colorRGB, unColorIntervals, ref nTempColorCount);
                    ...
                    else if (ColorType == AVG_UV)
                    {
                        ...
                        if (okFlag)
                        {
                            for (int k = 0; k < nCupNum; k++)
                            {
                                if (nChannelHs[1] > 0 && nChannelHs[2] > 0)
                                {
                                    CupImgDrawColor(ref bSrcImg, nW3, nCutLeft[k], nCutRight[k], nMinY[k], nMaxY[k], ucR, ucG, ucB);
                                    byte[] bSrcImgRight = new byte[nChannelHs[2] * nW3];
                                    Array.Copy(bSrcImg, (nChannelHs[0] + nChannelHs[1]) * nW3, bSrcImgRight, 0, nChannelHs[2] * nW3);
                                    CupImgDrawColor(ref bSrcImgRight, nW3, nCutLeft2[k], nCutRight2[k], nMinY2[k], nMaxY2[k], ucR, ucG, ucB);
                                    Array.Copy(bSrcImgRight, 0, bSrcImg, (nChannelHs[0] + nChannelHs[1]) * nW3, nChannelHs[2] * nW3);
                                }
                                else
                                {
                                    byte[] bSrcImgMid = new byte[nChannelHs[0] * nW3];
                                    Array.Copy(bSrcImg, nChannelHs[1] * nW3, bSrcImgMid, 0, nChannelHs[0] * nW3);
                                    CupImgDrawColor(ref bSrcImgMid, nW3, nCutLeft[k], nCutRight[k], nMinY[k], nMaxY[k], ucR, ucG, ucB);
                                    Array.Copy(bSrcImgMid, 0, bSrcImg, nChannelHs[1] * nW3, nChannelHs[0] * nW3);
                                }
                            }
                        }
        ...
        public static void ColorStatistic24_foreBin(ref byte[] bSrcImg, byte[] bBinImg, int nImageW, int[] nChannelHs, int nCupNum, int nMidLen, float fCutY, int ColorType,
         ColorRGB[] colorRGB, uint[] unColorIntervals, ref int[] ColorCount)
        {
            int[] nLefts1 = new int[ConstPreDefine.MAX_SPLICE_CUP_NUM];
            int[] nLefts0 = new int[ConstPreDefine.MAX_SPLICE_CUP_NUM];
            int[] nLefts2 = new int[ConstPreDefine.MAX_SPLICE_CUP_NUM];
            GetLefts_foreBin(bBinImg, nImageW, nChannelHs, ref nLefts1, ref nLefts0, ref nLefts2);
            ...
            try
            {
                ...
                for (int k = 0; k < nCupNum; k++)
                {
                    ...
                    {
                        //Left通道
                        GetCupImgOutRect_Gray(bBinImg, nImageW, nChannelHs[1], nLefts1[k], nLefts1[k + 1], ref nMinX[k], ref nMaxX[k], ref nMinY[k], ref nMaxY[k]);
                        nMaxY[k] = nMaxY[k] - (int)((nMaxY[k] - nMinY[k] + 1) * fCutY);
                        nCx = nMinX[k] + (nMaxX[k] - nMinX[k] >> 1);
                        if (k == 0)
                        {
                        ...
                        nCutLeft[k] = nCx - nMidLen;
                        nCutRight[k] = (nCx + nMidLen) < (nCutLeft[k] + nPerimeter - (k * nMidLen << 1)) ? (nCx + nMidLen) : (nCutLeft[k] + nPerimeter - (k * nMidLen << 1));
                        CupImgColorStatistic_ForeBin(ref bSrcImg, bBinImg, nImageW, nCutLeft[k], nCutRight[k], nMinY[k], nMaxY[k], ColorType, colorRGB, unColorIntervals, ref nTempColorCount);
                        if (ColorType == RATIO_H || ColorType == RATIO_Y || (ColorType == RATIO_UV))
                        {
                            ColorCount[0] += nTempColorCount[0];
                            ColorCount[1] += nTempColorCount[1];
                        ...
                        byte[] bSrcImgRight = new byte[nChannelHs[2] * nW3];
                        Array.Copy(bSrcImg, (nChannelHs[0] + nChannelHs[1]) * nW3, bSrcImgRight, 0, nChannelHs[2] * nW3);
                        byte[] bBinImgRight = new byte[nChannelHs[2] * nImageW];
                        Array.Copy(bBinImg, (nChannelHs[0] + nChannelHs[1]) * nImageW, bBinImgRight, 0, nChannelHs[2] * nImageW);
                        GetCupImgOutRect_Gray(bBinImgRight, nImageW, nChannelHs[2], nLefts2[k], nLefts2[k + 1], ref nMinX2[k], ref nMaxX2[k], ref nMinY2[k], ref nMaxY2[k]);
                        nMinY2[k] = (int)(nMinY2[k] + (nMaxY2[k] - nMinY2[k] + 1) * fCutY);
                        nCx = nMinX2[k] + (nMaxX2[k] - nMinX2[k] >> 1);

                        nCutLeft2[k] = nCx - nMidLen;
                        nCutRight2[k] = (nCx + nMidLen) < (nCutLeft2[k] + nPerimeter - (k * nMidLen << 1)) ? (nCx + nMidLen) : (nCutLeft2[k] + nPerimeter - (k * nMidLen << 1));
                        CupImgColorStatistic_ForeBin(ref bSrcImgRight, bBinImgRight, nImageW, nCutLeft2[k], nCutRight2[k], nMinY2[k], nMaxY2[k], ColorType, colorRGB, unColorIntervals, ref nTempColorCount);
                        Array.Copy(bSrcImgRight, 0, bSrcImg, (nChannelHs[0] + nChannelHs[1]) * nW3, nChannelHs[2] * nW3);
                        if (ColorType == RATIO_H || ColorType == RATIO_Y || (ColorType == RATIO_UV))
                        {
                            ColorCount[0] += nTempColorCount[0];
                    ...
                    else
                    {
                        ...
                        byte[] bSrcImgMid = new byte[nChannelHs[0] * nW3];
                        Array.Copy(bSrcImg, nChannelHs[1] * nW3, bSrcImgMid, 0, nChannelHs[0] * nW3);
                        byte[] bBinImgMid = new byte[nChannelHs[0] * nImageW];
                        Array.Copy(bBinImg, nChannelHs[1] * nImageW, bBinImgMid, 0, nChannelHs[0] * nImageW);
                        GetCupImgOutRect_Gray(bBinImgMid, nImageW, nChannelHs[0], nLefts0[k], nLefts0[k + 1], ref nMinX[k], ref nMaxX[k], ref nMinY[k], ref nMaxY[k]);
                        ...
                        nCutLeft[k] = nCx - nMidLen;
                        nCutRight[k] = (nCx + nMidLen) < (nCutLeft[k] + nPerimeter - (k * nMidLen << 1)) ? (nCx + nMidLen) : (nCutLeft[k] + nPerimeter - (k * nMidLen << 1));
                        CupImgColorStatistic_ForeBin(ref bSrcImgMid, bBinImgMid, nImageW, nCutLeft[k], nCutRight[k], nMinY[k], nMaxY[k], ColorType, colorRGB, unColorIntervals, ref nTempColorCount);
                        Array.Copy(bSrcImgMid, 0, bSrcImg, nChannelHs[1] * nW3, nChannelHs[0] * nW3);
                        if (ColorType == RATIO_H || ColorType == RATIO_Y || (ColorType == RATIO_UV))
                        {
                            ColorCount[0] += nTempColorCount[0];
                    ...
                    else if (ColorType == AVG_UV)
                    {
                        ...
                        if (okFlag)
                        {
                            for (int k = 0; k < nCupNum; k++)
                            {
                                if (nChannelHs[1] > 0 && nChannelHs[2] > 0)
                                {
                                    CupImgDrawColor_ForeBin(ref bSrcImg, bBinImg, nImageW, nCutLeft[k], nCutRight[k], nMinY[k], nMaxY[k], ucR, ucG, ucB);
                                    byte[] bSrcImgRight = new byte[nChannelHs[2] * nW3];
                                    Array.Copy(bSrcImg, (nChannelHs[0] + nChannelHs[1]) * nW3, bSrcImgRight, 0, nChannelHs[2] * nW3);
                                    byte[] bBinImgRight = new byte[nChannelHs[2] * nImageW];
                                    Array.Copy(bBinImg, (nChannelHs[0] + nChannelHs[1]) * nImageW, bBinImgRight, 0, nChannelHs[2] * nImageW);
                                    CupImgDrawColor_ForeBin(ref bSrcImgRight, bBinImgRight, nImageW, nCutLeft2[k], nCutRight2[k], nMinY2[k], nMaxY2[k], ucR, ucG, ucB);
                                    Array.Copy(bSrcImgRight, 0, bSrcImg, (nChannelHs[0] + nChannelHs[1]) * nW3, nChannelHs[2] * nW3);
                                }
                                else
                                {
                                    ...
                                    byte[] bBinImgMid = new byte[nChannelHs[0] * nImageW];
                                    Array.Copy(bBinImg, nChannelHs[1] * nImageW, bBinImgMid, 0, nChannelHs[0] * nImageW);
                                    CupImgDrawColor_ForeBin(ref bSrcImgMid, bBinImgMid, nImageW, nCutLeft[k], nCutRight[k], nMinY[k], nMaxY[k], ucR, ucG, ucB);
                                    Array.Copy(bSrcImgMid, 0, bSrcImg, nChannelHs[1] * nW3, nChannelHs[0] * nW3);
                                }
                            }
                        }
            ...
            {
                Trace.WriteLine("Commonfunction中函数ColorStatistic24_foreBin出错" + ex);
#if REALEASE
                GlobalDataInterface.WriteErrorInfo("Commonfunction中函数ColorStatistic出错" + ex);
#endif
            }
        }
        public static void CupImgColorStatistic_ForeBin(ref byte[] bSrcImg, byte[] bBinImg, int nImageW, int nMinX, int nMaxX, int nMinY, int nMaxY, int ColorType, ColorRGB[] colorRGB, uint[] unColorIntervals, ref int[] ColorTemp)
        {
            int x, y, nX3, nOffset, nOffset3, nImageW3, nH1, nH, nMinVal, nMaxVal, nB, nG, nR, nU, nV;
            byte ucY, ucU, ucV, ucR, ucG, ucB;
            ...
            nOffset = nMinY * nImageW;
            nOffset3 = nOffset * 3;
            for (y = nMinY; y <= nMaxY; y++, nOffset3 += nImageW3, nOffset += nImageW)
            {
                nX3 = nMinX * 3;
                for (x = nMinX; x <= nMaxX; x++, nX3 += 3)
                {
        ...
        public static void CupImgDrawColor_ForeBin(ref byte[] bSrcImg, byte[] bBinImg, int nImageW, int nMinX, int nMaxX, int nMinY, int nMaxY, byte ucR, byte ucG, byte ucB)
        {
            int x, y, nX3, nImageW3, nOffset, nOffset3;

            nImageW3 = nImageW * 3;
            nOffset = nMinY * nImageW;
            nOffset3 = nOffset * 3;
            for (y = nMinY; y <= nMaxY; y++, nOffset += nImageW, nOffset3 += nImageW3)
            {
                nX3 = nMinX * 3;
                for (x = nMinX; x <= nMaxX; x++, nX3 += 3)
                {
        ...
        /// 获取DataInterface的差值
        /// </summary>
        /// <param name="stStatistics1"></param>
        /// <param name="dataInterface2"></param>
        /// <returns></returns>
        public static DataInterface GetDifferDataInterface(stStatistics stStatistics1, DataInterface dataInterface2)
        {
            try
            {
                DataInterface differDataInterface = null;
                stStatistics differStStatistics = new stStatistics(true);
                //differStStatistics = dataInterface2.IoStStatistics;
                differStStatistics.ToCopy(dataInterface2.IoStStatistics);

                for (int i = 0; i < ConstPreDefine.MAX_QUALITY_GRADE_NUM * ConstPreDefine.MAX_SIZE_GRADE_NUM; i++)
                {
                    differStStatistics.nGradeCount[i] = dataInterface2.IoStStatistics.nGradeCount[i] - stStatistics1.nGradeCount[i];
                    differStStatistics.nWeightGradeCount[i] = dataInterface2.IoStStatistics.nWeightGradeCount[i] - stStatistics1.nWeightGradeCount[i];
                    differStStatistics.nBoxGradeCount[i] = dataInterface2.IoStStatistics.nBoxGradeCount[i] - stStatistics1.nBoxGradeCount[i];
                }

                for (int i = 0; i < ConstPreDefine.MAX_EXIT_NUM; i++)
                {
                    differStStatistics.nExitCount[i] = dataInterface2.IoStStatistics.nExitCount[i] - stStatistics1.nExitCount[i];
                    differStStatistics.nExitWeightCount[i] = dataInterface2.IoStStatistics.nExitWeightCount[i] - stStatistics1.nExitWeightCount[i];
                }

                differStStatistics.nTotalCount = dataInterface2.IoStStatistics.nTotalCount - stStatistics1.nTotalCount;
                differStStatistics.nWeightCount = dataInterface2.IoStStatistics.nWeightCount - stStatistics1.nWeightCount;

                differDataInterface = dataInterface2; //全局赋值
                //differDataInterface.IoStStatistics = differStStatistics; //局部赋值
                differDataInterface.IoStStatistics.ToCopy(differStStatistics);

                return differDataInterface;
            }
            catch (Exception ex)
            {
                Console.WriteLine("GetDifferDataInterface函数出错：" + ex.StackTrace + "\r\n" + ex.ToString());
                return null;
            }
        }
        ...
        public static ushort GetCsvFieldCount(string CsvStr, char CsvSapT)
        {
            ushort Result = 0;
            ...
            if (CsvStr[CsvStr.Length - 1] != CsvSapT)
            {
                CsvStr += CsvSapT;
            }
        ...
        public static bool GetCsvField(string CsvStr, ushort Index, ref string FieldStr, char CsvSapT)
        {
            ushort I = 0;
            int CommaPos = 0;
            bool Result = false;
            FieldStr = "";
            if (Index < GetCsvFieldCount(CsvStr, CsvSapT))
            {
                if (CsvStr[CsvStr.Length - 1] != CsvSapT)
                {
                    CsvStr += CsvSapT;
                ...
                do
                {
                    CommaPos = CsvStr.IndexOf(CsvSapT);
                    if (CommaPos >= 0)
                    {
                        FieldStr = CsvStr.Substring(0, CommaPos);
                        CsvStr = CsvStr.Substring(CommaPos + 1);
                    }
                    I++;
                } while (I <= Index);
            }
        ...
        public static string ExtractFileNames(string FileNames)
        {
            string S = "";
            int index = 0;
            while ((index = FileNames.IndexOf(".")) > 0)
            {
                S += FileNames.Substring(0, index - 1);
        ...
        {
            return Encoding.Default.GetBytes(PadRightEx(str, length, '\0'));
        }
        ...
        public static TMsgByte24Format StringToByteArrayConversion(string msg)
        {
            TMsgByte24Format buffer = new TMsgByte24Format(true);
            buffer.buf = StringToByteArray(msg, buffer.buf.Length);
            return buffer;
        }

        public static string ByteArrayToStringConversion(TMsgByte24Format buff)
        {
            return ByteArrayToString(buff.buf);
        }
